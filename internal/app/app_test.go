...
func TestNewWithConfigDefaultsManagedCPAPublicURLToBaseURL(t *testing.T) {
	cfg := testAppConfig(t)
	cfg.CPAPublicURL = ""
	cfg.CPAManagedEnabled = true
	cfg.CPABaseURL = "http://127.0.0.1:18218"
	if got := cpaPublicURL(cfg); got != "http://127.0.0.1:18218" {
		t.Fatalf("expected managed CPA base URL fallback, got %q", got)
	}
}
...
func readAppLogFile(t *testing.T, logDir string) string {
	t.Helper()
	path := filepath.Join(logDir, "CLIProxyAPIManager-"+time.Now().Format("2006-01-02")+".log")
