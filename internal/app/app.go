...
import (
	...
	"CLIProxyAPIManager/internal/logging"
...
type App struct {
	...
	AuthFileCooldowns Runner
	BackupMaintenance *DatabaseBackupRunner
	CPAManager        *cpamanager.Manager
...
func NewWithConfig(cfg config.Config) (*App, error) {
	...
	usageEventHub := service.NewUsageEventHub(service.DefaultUsageEventHubBufferSize)
	...
	syncService := service.NewSyncServiceWithOptions(db, service.SyncServiceOptions{
		...
		UsageEventPublisher: usageEventHub,
	})
	...
	if cfg.SetupRequired {
		metadataSyncRunner = nil
		redisIngestRunner = nil
		redisProcessRunner = nil
		backgroundPoller = nil
	}
	...
	proxyPoolService := service.NewProxyPoolService(db)
	authFilesManagementService := service.NewAuthFilesManagementServiceWithOnChanged(cpaClient, syncService.SyncMetadata)
	authFileCooldownService := service.NewAuthFileCooldownService(db, cpaClient, syncService.SyncMetadata)
	quotaService.SetLimitReachedCooldownStarter(authFileCooldownService)
	...
	authHandler := api.NewAuthHandler(api.AuthConfig{
		...
		SetupSaver:    cfg.SaveFirstRunSetup,
	}, sessionManager)
	var cpaManager *cpamanager.Manager
	if cfg.CPAManagedEnabled && !cfg.SetupRequired {
		cpaManager = cpamanager.New(cpamanager.Config{
			Enabled:             cfg.CPAManagedEnabled,
			WorkDir:             cfg.CPAWorkDir,
			ExePath:             cfg.CPAExePath,
			ConfigPath:          cfg.CPAConfigPath,
			ManagementKey:       cfg.CPAManagementKey,
			Port:                cfg.CPAPort(),
			AutoStart:           cfg.CPAAutoStart,
			UpdateCheckInterval: cfg.CPAUpdateCheckInterval,
			ReleaseRepo:         cfg.CPAReleaseRepo,
			RequestTimeout:      cfg.RequestTimeout,
		})
		if err := cpaManager.EnsureInstalled(context.Background()); err != nil {
			_ = closeGormDB(db)
			_ = logCloser.Close()
			return nil, fmt.Errorf("prepare managed CPA: %w", err)
		}
	}
	...
	return &App{
		...
		AuthFileCooldowns: authFileCooldownService,
		...
		Router: api.NewRouter(
			...
			api.OptionalProviders{
				...
				AuthCooldowns:  authFileCooldownService,
				ProxyPools:     proxyPoolService,
				CPAManager:     cpaManager,
				Config:         &cfg,
				Status:         api.StatusRouteConfig{CPAPublicURL: cpaPublicURL(cfg), ActiveRecorder: quotaActiveRecorder(cfg, quotaService), QuotaAutoRefreshEnabled: cfg.QuotaAutoRefreshEnabled},
				UsageEventsHub: usageEventHub,
			},
		),
	}, nil
}

func cpaPublicURL(cfg config.Config) string {
	if strings.TrimSpace(cfg.CPAPublicURL) != "" {
		return cfg.CPAPublicURL
	}
	if cfg.CPAManagedEnabled {
		return cfg.CPABaseURL
	}
	return ""
}
...
func (a *App) Close() error {
	...
	var closeErr error
	if a.CPAManager != nil {
		closeErr = errors.Join(closeErr, a.CPAManager.Stop(context.Background()))
		a.CPAManager = nil
	}
...
func (a *App) Run() error {
	...
	if a.CPAManager != nil && a.Config.CPAAutoStart {
		if err := a.CPAManager.Start(ctx); err != nil {
			logrus.Errorf("managed CPA start failed: %v", err)
		} else if err := waitForManagedCPAManagementReady(ctx, *a.Config); err != nil {
			logrus.Warnf("managed CPA management interface not ready before metadata sync: %v", err)
		}
	}
	...
	if a.AuthFileCooldowns != nil {
		a.startBackgroundTask(func() {
			if err := a.AuthFileCooldowns.Run(ctx); err != nil {
				logrus.Errorf("auth file cooldown runner stopped: %v", err)
			}
		})
	}
	...
	if a.Config.OpenBrowserOnStart {
		a.startBackgroundTask(func() {
			time.Sleep(500 * time.Millisecond)
			if err := openBrowser(a.localWebURL()); err != nil {
				logrus.Warnf("open browser failed: %v", err)
			}
		})
	}

	server := &http.Server{
		Addr:    a.Config.WebHost + ":" + a.Config.AppPort,
		Handler: a.Router,
	}
...
func (a *App) localWebURL() string {
	scheme := "http"
	if a.Config.TLSEnabled {
		scheme = "https"
	}
	host := strings.TrimSpace(a.Config.WebHost)
	if host == "" || host == "0.0.0.0" || host == "::" {
		host = "127.0.0.1"
	}
	return fmt.Sprintf("%s://%s:%s%s/", scheme, host, a.Config.AppPort, a.Config.AppBasePath)
}

func waitForManagedCPAManagementReady(ctx context.Context, cfg config.Config) error {
	if strings.TrimSpace(cfg.CPABaseURL) == "" || strings.TrimSpace(cfg.CPAManagementKey) == "" {
		return fmt.Errorf("CPA base URL or management key is empty")
	}
	...
	url := strings.TrimRight(cfg.CPABaseURL, "/") + "/v0/management/auth-files"
	var lastErr error
	for {
		...
		req.Header.Set("Authorization", "Bearer "+cfg.CPAManagementKey)
		...
		if err == nil {
			...
			lastErr = fmt.Errorf("management ready check returned status %d", resp.StatusCode)
		} else {
			lastErr = err
		}
		select {
		...
		case <-deadline.C:
			...
			return fmt.Errorf("timed out waiting for CPA management interface")
		}
	}
}
