...
func (s *Service) Refresh(ctx context.Context, request RefreshRequest) (RefreshResponse, error) {
	...
	response := RefreshResponse{
		Tasks:    make([]RefreshTaskRef, 0, limit),
...
func (s *Service) runRefreshTaskWithWorker(authIndex string) {
	...
	task := s.markRefreshTaskCompleted(authIndex, response)
	s.startCooldownForLimitReached(ctx, task, response)
}
...
func (s *Service) startCooldownForLimitReached(ctx context.Context, task *RefreshTaskRecord, response CheckResponse) {
	if s == nil || s.cooldownStarter == nil || task == nil || !refreshSourceAllowsCooldown(task.Source) {
		return
	}
	fileName := ""
	...
	if strings.TrimSpace(task.AuthIndex) == "" || fileName == "" {
		return
	}
	identity := entities.UsageIdentity{Identity: task.AuthIndex, Type: task.Type}
	if !inspectionQuotaLimitReached(identity, task, response.Quota) {
		return
	}
	cooldownDuration := authFileLimitReachedCooldownDuration(response.Quota, task.RefreshedAt)
	if err := s.cooldownStarter.StartLimitReachedCooldown(ctx, task.AuthIndex, fileName, string(task.Source), "limit_reached", cooldownDuration); err != nil {
		logrus.WithError(err).WithFields(logrus.Fields{
			"auth_index": task.AuthIndex,
			"file_name":  fileName,
			"source":     task.Source,
...
func refreshSourceAllowsCooldown(source RefreshSource) bool {
	switch source {
	case RefreshSourceManual, RefreshSourceAuto, RefreshSourceInspection, RefreshSourceScheduled:
		return true
	default:
		return false
	}
...
func (s *Service) markRefreshTaskCompleted(authIndex string, response CheckResponse) *RefreshTaskRecord {
