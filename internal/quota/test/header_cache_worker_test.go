...
func TestApplyUsageHeaderSnapshotStartsCooldownWhenLimitReached(t *testing.T) {
	db := openQuotaTestDatabase(t)
	...
	applied := applyUsageHeaderSnapshot(service, context.Background(), codexUsageHeaderSnapshot("codex-auth", time.Date(2026, 6, 22, 11, 0, 0, 0, time.Local), "100"))
	if !applied {
		t.Fatal("expected header snapshot to apply")
	}
	...
	if calls[0].authIndex != "codex-auth" || calls[0].fileName != "codex-user.json" || calls[0].source != string(RefreshSourceUsageHeader) || calls[0].reason != "limit_reached" || calls[0].duration != 4*time.Hour {
		t.Fatalf("unexpected cooldown call: %+v", calls[0])
	}
}

func TestApplyUsageHeaderSnapshotDoesNotStartCooldownForNormalUsage(t *testing.T) {
	db := openQuotaTestDatabase(t)
	seedUsageIdentity(t, db, entities.UsageIdentity{Identity: "codex-auth", Provider: "codex", Type: "codex", AuthType: entities.UsageIdentityAuthTypeAuthFile, FileName: quotaStringPtr("codex-user.json")})
	service := NewServiceWithRegistry(db, NewProviderRegistry(nil))
	defer service.StopRefreshTasks()
	...
	applied := applyUsageHeaderSnapshot(service, context.Background(), codexUsageHeaderSnapshot("codex-auth", time.Date(2026, 6, 22, 11, 0, 0, 0, time.Local), "99.9"))
	if !applied {
		t.Fatal("expected header snapshot to apply")
	}
	if calls := cooldowns.callSnapshot(); len(calls) != 0 {
		t.Fatalf("expected no cooldown call, got %+v", calls)
	}
}
