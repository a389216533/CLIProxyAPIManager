...
type LimitReachedCooldownStarter interface {
	StartLimitReachedCooldown(ctx context.Context, authIndex, fileName, source, reason string, duration time.Duration) error
}
...
type Service struct {
	...
	cooldownStarter LimitReachedCooldownStarter
}
...
func (s *Service) SetLimitReachedCooldownStarter(starter LimitReachedCooldownStarter) {
	if s == nil {
		return
	}
	s.cooldownStarter = starter
}
