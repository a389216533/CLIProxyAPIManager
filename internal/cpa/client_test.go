...
func TestFetchAuthFilesParsesSyncMetadataFields(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"files":[{"auth_index":"codex-auth","name":"codex-user.json","path":"/data/auths/codex-user.json","type":"codex","prefix":"team","workspace_name":"Workspace A","priority":7,"disabled":false,"note":"primary auth"},{"auth_index":"gemini-auth","type":"gemini"}]}`))
	}))
	defer server.Close()
	...
	if file.WorkspaceName != "Workspace A" {
		t.Fatalf("expected workspace name to decode, got %+v", file)
	}
...
func TestFetchAuthFilesParsesCodexIDTokenFields(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"files":[{"auth_index":"codex-auth","type":"codex","id_token":{"chatgpt_account_id":"acct_123","chatgpt_subscription_active_start":"2026-05-01T00:00:00Z","chatgpt_subscription_active_until":"2026-06-01T00:00:00Z","plan_type":"team","workspace_name":"Team Workspace"}}]}`))
	}))
	defer server.Close()
	...
	if file.IDToken.WorkspaceName == nil || *file.IDToken.WorkspaceName != "Team Workspace" {
		t.Fatalf("expected workspace name to decode, got %+v", file.IDToken.WorkspaceName)
	}
}

func TestFetchAuthFilesEnrichesLocalCodexMetadataFromAuthFile(t *testing.T) {
	...
	if err := os.WriteFile(authPath, []byte(`{
		...
		"id_token":"jwt-string-is-not-json-object"
	}`), 0o600); err != nil {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"files":[{"auth_index":"codex-auth","type":"codex","path":` + strconv.Quote(authPath) + `}]}`))
	}))
	defer server.Close()
	...
	file := result.Payload.Files[0]
	if file.AccountID != "acct_top_level" {
		t.Fatalf("expected account_id fallback, got %+v", file)
	}
	if file.CodexAuthMetadata.AccountID == nil || *file.CodexAuthMetadata.AccountID != "acct_chatgpt" {
		t.Fatalf("expected chatgpt account id fallback, got %+v", file.CodexAuthMetadata)
	}
	if file.CodexAuthMetadata.PlanType == nil || *file.CodexAuthMetadata.PlanType != "plus" {
		t.Fatalf("expected plan type fallback, got %+v", file.CodexAuthMetadata.PlanType)
	}
	if file.WorkspaceName != "Local Workspace" {
		t.Fatalf("expected workspace name fallback, got %+v", file)
	}
	if file.CodexAuthMetadata.ActiveUntil == nil || !file.CodexAuthMetadata.ActiveUntil.Equal(time.Date(2026, 6, 1, 0, 0, 0, 0, time.UTC)) {
		t.Fatalf("expected active until fallback, got %+v", file.CodexAuthMetadata.ActiveUntil)
	}
}
...
func TestUpdateAuthFileNotePatchesManagementEndpoint(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPatch {
			t.Fatalf("expected PATCH method, got %s", r.Method)
		}
		if r.URL.Path != cpaManagementAuthFilesNoteEndpoint {
			t.Fatalf("unexpected path %q", r.URL.Path)
		}
		if got := r.Header.Get("Authorization"); got != "Bearer management-secret" {
			t.Fatalf("expected management Authorization header, got %q", got)
		...
		if body["name"] != "codex-user.json" || body["note"] != "Team A" {
			t.Fatalf("unexpected note request body: %#v", body)
		}
	...
	client := NewClient("http://cpa.example.test", "management-secret", 2*time.Second, false)
	...
	note := "Team A"
	if err := client.UpdateAuthFileNote(context.Background(), "codex-user.json", &note); err != nil {
		t.Fatalf("UpdateAuthFileNote returned error: %v", err)
	}
}

func TestUpdateAuthFileNoteUpdatesLocalJSON(t *testing.T) {
	authFilePath := filepath.Join(t.TempDir(), "codex-user.json")
	if err := os.WriteFile(authFilePath, []byte(`{"type":"codex","note":"old"}`), 0o600); err != nil {
		t.Fatalf("write auth file: %v", err)
	}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != cpaManagementAuthFilesEndpoint {
			t.Fatalf("unexpected request %s %s", r.Method, r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"files":[{"auth_index":"codex-auth","name":"codex-user.json","path":` + strconv.Quote(authFilePath) + `,"type":"codex"}]}`))
	}))
	defer server.Close()
	...
	if payload["note"] != "Team A" {
		t.Fatalf("expected note to be updated, got %#v", payload)
	}

	if err := client.UpdateAuthFileNote(context.Background(), "codex-user.json", nil); err != nil {
		t.Fatalf("UpdateAuthFileNote clear returned error: %v", err)
	}
	data, err = os.ReadFile(authFilePath)
	if err != nil {
		t.Fatalf("read auth file after clear: %v", err)
	}
	payload = nil
	if err := json.Unmarshal(data, &payload); err != nil {
		t.Fatalf("decode auth file after clear: %v", err)
	}
	if _, ok := payload["note"]; ok {
		t.Fatalf("expected note to be cleared, got %#v", payload)
	}
}

func TestImportAuthFilePostsRawPayloadWithName(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("name") != "codex-user.json" {
			t.Fatalf("unexpected name query %q", r.URL.RawQuery)
		}
		if got := r.Header.Get("Authorization"); got != "Bearer management-secret" {
			t.Fatalf("expected management Authorization header, got %q", got)
		}
		...
		if body["type"] != "codex" || body["access_token"] != "access-token" {
	...
	client := NewClient(server.URL, "management-secret", 2*time.Second, false)
	if err := client.ImportAuthFile(context.Background(), " codex-user.json ", map[string]any{"type": "codex", "access_token": "access-token"}); err != nil {
		t.Fatalf("ImportAuthFile returned error: %v", err)
	}
}
