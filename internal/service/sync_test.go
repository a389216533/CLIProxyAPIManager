...
import (
	...
	"CLIProxyAPIManager/internal/cpa/dto/cpaapikeys"
...
type recordingUsageEventPublisher struct {
	eventIDs []int64
}
...
func (r *recordingUsageEventPublisher) PublishUsageEventID(eventID int64) UsageEventNotification {
	r.eventIDs = append(r.eventIDs, eventID)
	return UsageEventNotification{Sequence: int64(len(r.eventIDs)), EventID: eventID}
}
...
func TestProcessRedisUsageInboxPublishesUsageEventIDAfterTransactionCommit(t *testing.T) {
	db := openSyncTestDatabase(t)
	seedRedisInboxMessagesForTest(t, db, `{"timestamp":"2026-04-27T08:00:00Z","provider":"claude","auth_type":"oauth","source":"auth-user@example.com","auth_index":"auth-1","model":"sonnet","request_id":"publish-event-id","tokens":{"input_tokens":1,"output_tokens":2}}`)
	publisher := &recordingUsageEventPublisher{}
	service := NewSyncServiceWithOptions(db, SyncServiceOptions{
		BaseURL:             "https://cpa.example.com",
	...
	if len(publisher.eventIDs) != 1 || publisher.eventIDs[0] == 0 {
		t.Fatalf("expected one committed usage event ID notification, got %+v", publisher.eventIDs)
	}
	var event entities.UsageEvent
	if err := db.First(&event, publisher.eventIDs[0]).Error; err != nil {
		t.Fatalf("expected published ID to load committed event: %v", err)
	}
	if event.EventKey != "publish-event-id" {
...
func TestProcessRedisUsageInboxDoesNotPublishUsageEventIDOnRollback(t *testing.T) {
	db := openSyncTestDatabase(t)
	seedRedisInboxMessagesForTest(t, db, `{"timestamp":"2026-04-27T08:00:00Z","provider":"claude","model":"sonnet","request_id":"rollback-publish-event-id","tokens":{"input_tokens":1,"output_tokens":2}}`)
	if err := db.Exec(`CREATE TRIGGER fail_publish_event_mark BEFORE UPDATE OF status ON redis_usage_inboxes WHEN NEW.status = 'processed' BEGIN SELECT RAISE(ABORT, 'processed mark failed'); END;`).Error; err != nil {
		t.Fatalf("create failure trigger: %v", err)
	}
	publisher := &recordingUsageEventPublisher{}
	service := NewSyncServiceWithOptions(db, SyncServiceOptions{
	...
	if len(publisher.eventIDs) != 0 {
		t.Fatalf("expected no usage event notification on rollback, got %+v", publisher.eventIDs)
	}
}
...
func TestSyncMetadataWritesCodexAuthFileIDTokenFieldsOnlyForCodex(t *testing.T) {
	...
	topLevelActiveUntil := time.Date(2026, 7, 1, 0, 0, 0, 0, time.UTC)
	accountID := "acct_123"
	ignoredAccountID := "acct_should_ignore"
	planType := "team"
	workspaceName := "Team Workspace"
	topLevelAccountID := "acct_top_level"
	topLevelPlanType := "plus"
	service := NewSyncServiceWithOptions(db, SyncServiceOptions{
		BaseURL: "https://cpa.example.com",
		MetadataFetcher: stubMetadataFetcher{authFilesResult: &response.AuthFilesResult{StatusCode: 200, Payload: authfiles.AuthFilesResponse{Files: []authfiles.AuthFile{{
			...
			Type:      "codex",
			Provider:  "Codex",
			CodexAuthMetadata: authfiles.CodexAuthMetadata{
				AccountID:   &topLevelAccountID,
				ActiveUntil: &topLevelActiveUntil,
				PlanType:    &topLevelPlanType,
			},
		}}}}},
	})
	...
	if codex.AccountID == nil || *codex.AccountID != "acct_123" || codex.PlanType == nil || *codex.PlanType != "team" || codex.WorkspaceName == nil || *codex.WorkspaceName != "Team Workspace" || codex.ActiveStart == nil || !codex.ActiveStart.Equal(activeStart) || codex.ActiveUntil == nil || !codex.ActiveUntil.Equal(activeUntil) {
		t.Fatalf("expected codex id_token fields to persist, got %+v", codex)
	}
	claude := byIdentity["claude-auth"]
	if claude.AccountID != nil || claude.PlanType != nil || claude.WorkspaceName != nil || claude.ActiveStart != nil || claude.ActiveUntil != nil {
		t.Fatalf("expected non-codex auth file to ignore id_token fields, got %+v", claude)
	}
	codexNoToken := byIdentity["codex-no-token"]
	if codexNoToken.AccountID == nil || *codexNoToken.AccountID != "acct_top_level" || codexNoToken.PlanType == nil || *codexNoToken.PlanType != "plus" || codexNoToken.ActiveUntil == nil || !codexNoToken.ActiveUntil.Equal(topLevelActiveUntil) {
		t.Fatalf("expected codex top-level metadata fields to persist, got %+v", codexNoToken)
	}
}
