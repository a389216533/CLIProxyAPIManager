...
func (s *usageService) ListUsageEvents(_ context.Context, filter servicedto.UsageFilter) (*servicedto.UsageEventsPage, error) {
	...
	for _, row := range page.Events {
		result = append(result, usageEventRecordFromRepository(row))
	}
...
// ListUsageEventsByIDs 使用 Request Event Log 相同过滤条件加载指定事件，供实时流按事件 ID 水合展示 payload。
func (s *usageService) ListUsageEventsByIDs(ctx context.Context, filter servicedto.UsageFilter, ids []int64) ([]servicedto.UsageEventRecord, error) {
	apiGroupKey, err := s.resolveAPIGroupKey(filter.APIKeyID)
	if err != nil {
		return nil, err
	}
	rows, err := repository.ListUsageEventsByIDsWithFilter(s.db.WithContext(ctx), ids, repodto.UsageQueryFilter{
		StartTime:   filter.StartTime,
		EndTime:     filter.EndTime,
		Model:       filter.Model,
		AuthIndex:   filter.AuthIndex,
	...
	result := make([]servicedto.UsageEventRecord, 0, len(rows))
...
func (s *usageService) StreamUsageEvents(ctx context.Context, filter servicedto.UsageFilter, emit func(servicedto.UsageEventRecord) error) error {
	...
	return repository.StreamUsageEventsWithFilter(s.db.WithContext(ctx), repodto.UsageQueryFilter{
		...
		return emit(usageEventRecordFromRepository(row))
	})
}

func usageEventRecordFromRepository(row repodto.UsageEventRecord) servicedto.UsageEventRecord {
