...
// ListUsageEventsByIDsWithFilter 加载指定 usage event ID，并复用 Request Event Log 的过滤和展示投影。
func ListUsageEventsByIDsWithFilter(db *gorm.DB, ids []int64, filter dto.UsageQueryFilter) ([]dto.UsageEventRecord, error) {
	if db == nil {
		return nil, fmt.Errorf("database is nil")
	}
	...
	query := applyUsageEventListQuery(db.Model(&entities.UsageEvent{}), filter)
	query = query.Where("id IN ?", cleanIDs).Select(usageEventProjectionColumns).Order("timestamp DESC, id DESC")
	return loadUsageEventRecordsForQuery(db, query)
}
...
func usageOverviewHealthWindow(filter dto.UsageQueryFilter, totalBlocks int, span time.Duration) (time.Time, time.Time) {
	...
	if isUsageOverviewCalendarDayHealthRange(filter.Range) && filter.StartTime != nil {
		...
		if !dayEnd.After(queryNow) {
			// 完整过去自然日保持自然日轴；今天不能展示到未来的午夜。
			return start, dayEnd
		}
	}
	if isUsageOverviewShortHealthRange(filter.Range) {
