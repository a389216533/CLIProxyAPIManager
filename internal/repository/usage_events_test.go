...
func TestListUsageEventsByIDsWithFilterAppliesListFilters(t *testing.T) {
	db, err := OpenDatabase(config.Config{SQLitePath: filepath.Join(t.TempDir(), "usage-events-by-ids.db")})
	if err != nil {
		t.Fatalf("OpenDatabase returned error: %v", err)
	}
	closeTestDatabase(t, db)
	base := time.Date(2026, 4, 20, 10, 0, 0, 0, time.Local)
	events := []entities.UsageEvent{
		{EventKey: "match-old", Model: "claude-sonnet", Timestamp: base.Add(-time.Minute), AuthIndex: "auth-1", TotalTokens: 10},
		{EventKey: "wrong-model", Model: "gpt-5", Timestamp: base, AuthIndex: "auth-1", TotalTokens: 20},
		{EventKey: "match-new", Model: "claude-sonnet", Timestamp: base.Add(time.Minute), AuthIndex: "auth-1", TotalTokens: 30},
	}
	if _, _, err := InsertUsageEvents(db, events); err != nil {
		t.Fatalf("InsertUsageEvents returned error: %v", err)
	}
	var inserted []entities.UsageEvent
	if err := db.Order("id ASC").Find(&inserted).Error; err != nil {
		t.Fatalf("load inserted events: %v", err)
	}
	start := base.Add(-2 * time.Minute)
	end := base.Add(2 * time.Minute)
	rows, err := ListUsageEventsByIDsWithFilter(db, []int64{inserted[0].ID, inserted[1].ID, inserted[2].ID}, dto.UsageQueryFilter{
		StartTime: &start,
		EndTime:   &end,
		Model:     "claude-sonnet",
	})
	if err != nil {
		t.Fatalf("ListUsageEventsByIDsWithFilter returned error: %v", err)
	}
	if len(rows) != 2 {
		t.Fatalf("expected two matching rows, got %+v", rows)
	}
	if rows[0].ID != inserted[2].ID || rows[1].ID != inserted[0].ID {
		t.Fatalf("expected matching rows in newest-first order, got %+v", rows)
	}
}
