...
const usageIdentityReadColumns = "id, name, alias, auth_type, auth_type_name, identity, type, provider, lookup_key, prefix, base_url, file_name, file_path, proxy_url, priority, disabled, note, account_id, project_id, workspace_name, active_start, active_until, plan_type, total_requests, success_count, failure_count, input_tokens, output_tokens, reasoning_tokens, cached_tokens, total_tokens, last_aggregated_usage_event_id, first_used_at, last_used_at, stats_updated_at, is_deleted, created_at, updated_at, deleted_at"
...
func ListActiveUsageIdentitiesPage(ctx context.Context, db *gorm.DB, request ListUsageIdentitiesPageRequest) ([]entities.UsageIdentity, int64, []dto.UsageIdentityTypeCount, error) {
	...
	proxyURLs := normalizeProxyURLs(request.ProxyURLs)
	...
	query = applyUsageIdentityProxyURLFilter(query, proxyURLs)
	if request.Query != "" {
		likeQuery := "%" + strings.TrimSpace(request.Query) + "%"
		query = query.Where("(name LIKE ? OR alias LIKE ? OR identity LIKE ? OR file_name LIKE ? OR proxy_url LIKE ?)", likeQuery, likeQuery, likeQuery, likeQuery, likeQuery)
	}
	var total int64
...
func activeUsageIdentitiesPageBaseQuery(db *gorm.DB, authType *entities.UsageIdentityAuthType, activeOnly *bool) *gorm.DB {
	...
	if activeOnly != nil && *activeOnly {
		query = query.Where("(disabled IS NULL OR disabled = ? OR EXISTS (SELECT 1 FROM auth_file_cooldowns WHERE auth_file_cooldowns.auth_index = usage_identities.identity AND auth_file_cooldowns.status = ?))", false, entities.AuthFileCooldownStatusActive)
	}
	return query
}
...
func normalizeUsageIdentities(identities []entities.UsageIdentity, authType entities.UsageIdentityAuthType) ([]entities.UsageIdentity, []string) {
	...
	for _, identity := range identities {
		...
		identity.ProxyURL = trimOptionalString(identity.ProxyURL)
		...
		identity.WorkspaceName = trimOptionalString(identity.WorkspaceName)
...
func normalizeProxyURLs(proxyURLs []string) []string {
	...
	normalized := make([]string, 0, len(proxyURLs))
...
func applyUsageIdentityProxyURLFilter(query *gorm.DB, proxyURLs []string) *gorm.DB {
	switch len(proxyURLs) {
	case 0:
		return query
	case 1:
		return query.Where("proxy_url = ?", proxyURLs[0])
	default:
		return query.Where("proxy_url IN ?", proxyURLs)
	}
}
