...
func TestOrderedMigrationsPreservesExecutionOrder(t *testing.T) {
	...
	want := []string{
		...
		"20260701_add_proxy_pools",
		"20260702_add_usage_identity_workspace_name",
		"20260702_create_auth_file_cooldowns",
	}
	if len(got) != len(want) {
...
func TestOpenDatabaseRunsSchemaMigrationsAndAddsUsageEventRedisFields(t *testing.T) {
	...
	if !db.Migrator().HasColumn(&entities.UsageIdentity{}, "proxy_url") {
		t.Fatal("expected usage_identities.proxy_url column to exist")
	}
	if !db.Migrator().HasTable(&entities.ProxyPool{}) {
		t.Fatal("expected proxy_pools table to exist")
	}
	...
	if !db.Migrator().HasTable(&entities.AuthFileCooldown{}) {
		t.Fatal("expected auth_file_cooldowns table to exist")
	}
