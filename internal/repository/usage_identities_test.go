...
func TestUsageIdentityReplaceForAuthTypePersistsSourceMetadataFields(t *testing.T) {
	...
	if updated.AccountID == nil || *updated.AccountID != "acct_123" || updated.PlanType == nil || *updated.PlanType != "team" || updated.WorkspaceName == nil || *updated.WorkspaceName != "Codex Workspace" || updated.ActiveStart == nil || !updated.ActiveStart.Equal(activeStart) || updated.ActiveUntil == nil || !updated.ActiveUntil.Equal(activeUntil) {
...
func TestUsageIdentityListActivePageKeepsCooldownAuthFilesVisible(t *testing.T) {
	db := openTestDatabase(t)
	now := time.Date(2026, 7, 2, 11, 0, 0, 0, time.UTC)
	disabled := true
	rows := []entities.UsageIdentity{
		{Identity: "enabled", Name: "Enabled", AuthType: entities.UsageIdentityAuthTypeAuthFile, AuthTypeName: "oauth", Type: "claude", Provider: "Claude", Disabled: nil, TotalRequests: 10, CreatedAt: now, UpdatedAt: now},
		{Identity: "cooldown", Name: "Cooldown", AuthType: entities.UsageIdentityAuthTypeAuthFile, AuthTypeName: "oauth", Type: "claude", Provider: "Claude", Disabled: &disabled, TotalRequests: 20, CreatedAt: now, UpdatedAt: now},
		{Identity: "disabled", Name: "Disabled", AuthType: entities.UsageIdentityAuthTypeAuthFile, AuthTypeName: "oauth", Type: "claude", Provider: "Claude", Disabled: &disabled, TotalRequests: 30, CreatedAt: now, UpdatedAt: now},
	}
	if err := db.Create(&rows).Error; err != nil {
		t.Fatalf("seed usage identities: %v", err)
	}
	if err := db.Create(&entities.AuthFileCooldown{AuthIndex: "cooldown", FileName: "cooldown.json", Source: "auto", Reason: "limit_reached", Status: entities.AuthFileCooldownStatusActive, DisabledAt: now, RestoreAt: now.Add(5 * time.Hour)}).Error; err != nil {
		t.Fatalf("seed auth file cooldown: %v", err)
	}
	authType := entities.UsageIdentityAuthTypeAuthFile
	activeOnly := true

	items, total, _, err := ListActiveUsageIdentitiesPage(context.Background(), db, ListUsageIdentitiesPageRequest{AuthType: &authType, ActiveOnly: &activeOnly, Page: 1, PageSize: 10})
	if err != nil {
		t.Fatalf("list page: %v", err)
	}
	if total != 2 {
	...
	if got := []string{items[0].Identity, items[1].Identity}; !reflect.DeepEqual(got, []string{"cooldown", "enabled"}) {
		t.Fatalf("expected active cooldown auth file to remain visible, got %v", got)
	}
}
