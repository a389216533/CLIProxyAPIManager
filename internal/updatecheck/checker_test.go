...
func TestCheckerUsesLatestRelease(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/a389216533/CLIProxyAPIManager/releases/latest" {
...
func TestCheckerFallsBackToTags(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/a389216533/CLIProxyAPIManager/releases/latest":
			http.NotFound(w, r)
		case "/repos/a389216533/CLIProxyAPIManager/tags":
			w.Header().Set("Content-Type", "application/json")
