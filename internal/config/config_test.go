...
var configEnvKeys = []string{
	...
	"WEB_HOST", "CPA_MANAGED_ENABLED", "CPA_WORK_DIR", "CPA_EXE_PATH", "CPA_CONFIG_PATH", "CPA_AUTO_START", "CPA_UPDATE_CHECK_INTERVAL", "CPA_RELEASE_REPO", "CPA_MANAGED_SECRET_KEY", "MANAGER_LOGIN_PASSWORD",
...
func TestLoadFromEnvAppliesDefaults(t *testing.T) {
	...
	if cfg.AppPort != "18217" {
		t.Fatalf("expected default app port 18217, got %s", cfg.AppPort)
	}
	if cfg.WebHost != "0.0.0.0" {
		t.Fatalf("expected default web host 0.0.0.0, got %s", cfg.WebHost)
	}
	if cfg.AppBasePath != "" {
	...
	if !cfg.AuthEnabled {
		t.Fatal("expected auth to be enabled by default in managed mode")
	}
	if !cfg.CPAManagedEnabled {
		t.Fatal("expected managed CPA to be enabled by default")
	}
...
func TestLoadFromEnvManagedModeRequiresFirstRunSetupWhenSecretsAreMissing(t *testing.T) {
	...
	if !cfg.SetupRequired {
		t.Fatal("expected setup to be required when managed mode secrets are missing")
	}
	if !cfg.AuthEnabled {
		t.Fatal("expected auth to be enabled during first-run setup")
	}
	if cfg.CPABaseURL != "http://127.0.0.1:18218" {
		t.Fatalf("expected managed CPA base URL default 18218, got %q", cfg.CPABaseURL)
	}
	if cfg.LoginPassword != "" || cfg.CPAManagementKey != "" {
		t.Fatalf("expected missing setup secrets to stay empty, got password=%q key=%q", cfg.LoginPassword, cfg.CPAManagementKey)
	}
}

func TestLoadFromEnvTreatsTemplateSecretsAsMissing(t *testing.T) {
	withIsolatedEnvFiles(t)
	t.Setenv("CPA_MANAGEMENT_KEY", "replace-with-your-management-key")
	t.Setenv("LOGIN_PASSWORD", "replace-with-your-login-password")
	...
	if !cfg.SetupRequired {
		t.Fatal("expected setup to be required for template placeholder secrets")
	}
	if cfg.CPAManagementKey != "" || cfg.LoginPassword != "" {
		t.Fatalf("expected placeholder secrets to be blank, got password=%q key=%q", cfg.LoginPassword, cfg.CPAManagementKey)
	}
}
...
func TestSaveFirstRunSetupUpdatesOnlySetupSecrets(t *testing.T) {
	envPath := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(envPath, []byte("APP_PORT=19090\nMANAGER_LOGIN_PASSWORD=\nCPA_MANAGED_SECRET_KEY=\nWORK_DIR=./data\n"), 0600); err != nil {
		t.Fatalf("write env: %v", err)
	}
	cfg := Config{EnvFile: envPath}

	if err := cfg.SaveFirstRunSetup("secret123", "cpa-secret-123"); err != nil {
	...
	text := string(content)
	for _, expected := range []string{"APP_PORT=19090", "MANAGER_LOGIN_PASSWORD=secret123", "CPA_MANAGED_SECRET_KEY=cpa-secret-123", "WORK_DIR=./data"} {
...
func TestSaveFirstRunSetupAppendsMissingSetupSecrets(t *testing.T) {
	envPath := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(envPath, []byte("APP_PORT=19090\n"), 0600); err != nil {
		t.Fatalf("write env: %v", err)
	}
	...
	text := string(content)
	if !strings.Contains(text, "MANAGER_LOGIN_PASSWORD=secret123") || !strings.Contains(text, "CPA_MANAGED_SECRET_KEY=cpa-secret-123") {
		t.Fatalf("expected setup secrets to be appended, got:\n%s", text)
	}
}
...
func TestLoadFromEnvRequiresCriticalValues(t *testing.T) {
	...
	t.Run("missing base url", func(t *testing.T) {
		t.Setenv("CPA_MANAGED_ENABLED", "false")
		t.Setenv("CPA_MANAGEMENT_KEY", "secret")
