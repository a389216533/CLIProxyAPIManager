...
import { appPath, buildUsageEventsStreamURL, createCpaApiKey, deleteAuthFiles, deleteCpaApiKey, exportUsageEvents, fetchAnalysis, fetchAuthSessions, fetchAuthFileCooldowns, fetchConfigDiagnostics, fetchConfigStatus, fetchCpaApiKeyOptions, fetchCpaApiKeys, fetchCpaApiKeySettings, fetchCpaRuntime, fetchCpaUpdateEvents, fetchKeyOverview, fetchKeyOverviewRealtime, fetchUsageOverview, fetchUsageOverviewRealtime, fetchUsageQuotaCache, fetchUsageQuotaInspectionStatus, fetchUpdateCheck, fetchUsageEventModelFilterOptions, fetchUsageEventSourceFilterOptions, fetchUsageEvents, fetchUsageIdentities, fetchUsageIdentitiesPage, fetchUsageQuotaRefreshTask, fetchVersion, importAuthFilesFromToken, loginWithCPAAPIKey, logout, markStatusActive, refreshUsageQuotas, resetUsageQuota, restartCpaRuntime, restoreAuthFileCooldown, revokeAuthSession, setAuthFilesDisabled, setAuthFilesNote, startAuthFileCooldown, startCpaRuntime, startUsageQuotaInspection, stopCpaRuntime, updateCpaApiKey, updateCpaApiKeyAlias, updateCpaRuntime } from './api';
...
describe('fetchUsageEvents', () => {
  ...
  it('loads config status and diagnostics from dedicated no-store endpoints', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, status: 'ready', checks: [] }),
    } as Response);
    const signal = new AbortController().signal;

    await fetchConfigStatus(signal);
    await fetchConfigDiagnostics(signal);

    const statusUrl = new URL(String(fetchMock.mock.calls[0][0]), 'http://localhost');
    const diagnosticsUrl = new URL(String(fetchMock.mock.calls[1][0]), 'http://localhost');
    expect(statusUrl.pathname).toBe('/api/v1/config/status');
    expect(diagnosticsUrl.pathname).toBe('/api/v1/config/diagnostics');
    expect(fetchMock.mock.calls[0][1]).toMatchObject({ credentials: 'include', signal, cache: 'no-store' });
  ...
  it('builds usage events stream URL with filters but without pagination params', () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: '/keeper/' });

    const url = buildUsageEventsStreamURL('custom', '2026-04-20T00:00:00Z', '2026-04-21T00:00:00Z', {
      page: 3,
      pageSize: 100,
      model: 'claude-sonnet',
      source: 'authidx-source-a',
    ...
    expect(parsed.pathname).toBe('/keeper/api/v1/usage/events/stream');
    expect(parsed.searchParams.get('range')).toBe('custom');
    expect(parsed.searchParams.get('start')).toBe('2026-04-20T00:00:00Z');
    expect(parsed.searchParams.get('end')).toBe('2026-04-21T00:00:00Z');
    expect(parsed.searchParams.get('model')).toBe('claude-sonnet');
  ...
  it('creates, updates, and deletes CPA API keys', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: '123', apiKey: 'sk-created123456', keyAlias: 'Created', displayKey: 'sk-*********123456', label: 'Created', lastSyncedAt: null }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: '123', apiKey: 'sk-updated654321', keyAlias: 'Updated', displayKey: 'sk-*********654321', label: 'Updated', lastSyncedAt: null }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
      } as Response);

    const created = await createCpaApiKey({ keyAlias: 'Created', apiKey: 'sk-created123456' });
    const updated = await updateCpaApiKey('123', { keyAlias: 'Updated', apiKey: 'sk-updated654321' });
    await deleteCpaApiKey('123');

    const [createUrl, createInit] = fetchMock.mock.calls[0];
    const [updateUrl, updateInit] = fetchMock.mock.calls[1];
    const [deleteUrl, deleteInit] = fetchMock.mock.calls[2];

    expect(created.apiKey).toBe('sk-created123456');
    expect(updated.label).toBe('Updated');
    expect(new URL(String(createUrl), 'http://localhost').pathname).toBe('/api/v1/usage/api-keys');
    expect(createInit).toMatchObject({ credentials: 'include', method: 'POST' });
    expect(createInit?.body).toBe(JSON.stringify({ keyAlias: 'Created', apiKey: 'sk-created123456' }));
    expect(new URL(String(updateUrl), 'http://localhost').pathname).toBe('/api/v1/usage/api-keys/123');
    expect(updateInit).toMatchObject({ credentials: 'include', method: 'PUT' });
    expect(updateInit?.body).toBe(JSON.stringify({ keyAlias: 'Updated', apiKey: 'sk-updated654321' }));
    expect(new URL(String(deleteUrl), 'http://localhost').pathname).toBe('/api/v1/usage/api-keys/123');
  ...
  it('读取当前页 auth_index 的缓存限额且不触发刷新', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({
        items: [{ auth_index: 'auth-1', file_name: 'claude-user.json', status: 'completed', quota: { id: 'auth-1', quota: [{ key: 'rate_limit.secondary_window', label: '每周', remaining: 12 }] }, refreshed_at: '2026-05-25T00:00:00Z' }],
      }),
    } as Response);
    const signal = new AbortController().signal;
  ...
  it('兼容限额缓存里的 snake_case 响应字段', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({
    ...
    const response = await fetchUsageQuotaCache(['auth-1']);
    const quota = response.items[0].quota;

    expect(quota?.rateLimitResetCreditsAvailableCount).toBe(2);
    expect(quota?.quota[0]).toMatchObject({
      planType: 'pro',
      usedPercent: 25,
  ...
  it('为当前页 auth_index 创建限额刷新任务', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({
        tasks: [{ auth_index: 'auth-1' }],
        rejected: [{ auth_index: 'auth-2', error: 'duplicate' }],
        accepted: 1,
        skipped: 0,
        limit: 1,
      }),
    ...
    expect(response.tasks[0]).toMatchObject({ authIndex: 'auth-1' });
    expect(response.rejected[0]).toMatchObject({ authIndex: 'auth-2', error: 'duplicate' });
  ...
  it('兼容限额刷新响应中的空 tasks/rejected', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({
  ...
  it('兼容限额重置里的 snake_case 响应字段', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ auth_index: 'auth-1', windows_reset: 2 }),
    } as Response);

    const response = await resetUsageQuota('auth-1');

    expect(response.authIndex).toBe('auth-1');
    expect(response.windowsReset).toBe(2);
  });
  ...
  it('manages auth file cooldowns through protected endpoints', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ cooldowns: [{ id: 1, auth_index: 'auth-1', file_name: 'a.json', status: 'active' }] }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 1, auth_index: 'auth-1', file_name: 'a.json', status: 'active' }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 1, auth_index: 'auth-1', file_name: 'a.json', status: 'restored' }),
      } as Response);

    const cooldowns = await fetchAuthFileCooldowns();
    const started = await startAuthFileCooldown('auth-1', 'a.json');
    const restored = await restoreAuthFileCooldown('auth-1');

    expect(cooldowns).toHaveLength(1);
    expect(started.status).toBe('active');
    expect(restored.status).toBe('restored');
    expect(new URL(String(fetchMock.mock.calls[0][0]), 'http://localhost').pathname).toBe('/api/v1/auth-files/cooldowns');
    expect(fetchMock.mock.calls[1][1]).toMatchObject({ credentials: 'include', method: 'POST' });
    expect(fetchMock.mock.calls[1][1]?.body).toBe(JSON.stringify({ auth_index: 'auth-1', file_name: 'a.json' }));
    expect(new URL(String(fetchMock.mock.calls[2][0]), 'http://localhost').pathname).toBe('/api/v1/auth-files/cooldowns/restore');
    expect(fetchMock.mock.calls[2][1]?.body).toBe(JSON.stringify({ auth_index: 'auth-1' }));
  });
  ...
  it('updates auth file notes through the protected management endpoint', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ names: ['a.json'], affected: 1 }),
    ...
    const response = await setAuthFilesNote(['a.json'], 'Team A');

    const [url, init] = fetchMock.mock.calls[0];
    const parsed = new URL(String(url), 'http://localhost');

    expect(response.names).toEqual(['a.json']);
    expect(parsed.pathname).toBe('/api/v1/auth-files/note');
    expect(init).toMatchObject({ credentials: 'include', method: 'PATCH' });
    expect(init?.headers).toEqual({ 'Content-Type': 'application/json' });
    expect(init?.body).toBe(JSON.stringify({ names: ['a.json'], note: 'Team A' }));
  });

  it('imports auth files from pasted token content', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ names: ['codex-user.json'], affected: 1 }),
    } as Response);

    const response = await importAuthFilesFromToken('token-content');

    const [url, init] = fetchMock.mock.calls[0];
    const parsed = new URL(String(url), 'http://localhost');

    expect(response.names).toEqual(['codex-user.json']);
    expect(parsed.pathname).toBe('/api/v1/auth-files/import');
    expect(init).toMatchObject({ credentials: 'include', method: 'POST' });
    expect(init?.headers).toEqual({ 'Content-Type': 'application/json' });
    expect(init?.body).toBe(JSON.stringify({ content: 'token-content' }));
  });
  ...
  it('loads and controls the bundled CPA runtime', async () => {
    vi.stubGlobal('window', { __APP_BASE_PATH__: undefined });
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ enabled: true, running: true, updateAvailable: false }),
    } as Response);

    await fetchCpaRuntime();
    await startCpaRuntime();
    await stopCpaRuntime();
    await restartCpaRuntime();
    await updateCpaRuntime();
    await fetchCpaUpdateEvents();
    ...
    expect(paths).toEqual([
      ...
      '/api/v1/cpa/restart',
      '/api/v1/cpa/update',
      '/api/v1/cpa/update-events',
    ]);
    expect(fetchMock.mock.calls.slice(1, 5).map(([, init]) => init?.method)).toEqual(['POST', 'POST', 'POST', 'POST']);
  });
});
