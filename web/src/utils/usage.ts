...
export function formatCompactNumber(value: number): string {
  ...
  const formatUnit = (base: number, unit: string) => {
    const scaled = value / base;
    return `${new Intl.NumberFormat('zh-CN', { maximumFractionDigits: 1, useGrouping: false }).format(scaled)}${unit}`;
  };

  if (abs >= 100_000_000) {
    return formatUnit(100_000_000, '亿');
  }
  if (abs >= 10_000) {
    return formatUnit(10_000, '万');
  }
