...
import { generateCpaManagementKey, getLoginErrorForMode } from './LoginPage';
...
describe('LoginPage mode-specific errors', () => {
  ...
  it('generates a CPA management key suitable for first-run setup', () => {
    const key = generateCpaManagementKey();

    expect(key).toHaveLength(32);
    expect(key).toMatch(/^[A-HJ-NP-Za-km-z2-9]+$/);
  });
});
