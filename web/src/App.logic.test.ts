...
describe('App role route normalization', () => {
  ...
  it('mounts the shared footer only when the current page is not the usage dashboard', () => {
    expect(appSource).toContain("import './App.css';");
    expect(appSource).toContain("import { AppFooter } from './components/AppFooter';");
    expect(appSource).toContain('let showFooter = true;');
    expect(appSource).toMatch(/showFooter = false;[\s\S]*<UsagePage mode="public"/);
    expect(appSource).toMatch(/showFooter = false;[\s\S]*<UsagePage onAuthRequired=\{clearSession\}/);
    expect(appSource).toMatch(/\{showFooter && <AppFooter loadVersion=\{authState === 'authenticated'\} \/>\}/);
  });

  it('lets app pages fill the space above the shared footer', () => {
    expect(appStylesSource).toMatch(/\.app-frame\s*\{[\s\S]*?width:\s*100%;/);
    expect(appStylesSource).toMatch(/\.app-frame\s*\{[\s\S]*?min-width:\s*0;/);
    expect(appStylesSource).toMatch(/\.app-main\s*\{[\s\S]*?width:\s*100%;/);
    expect(appStylesSource).toMatch(/\.app-main\s*\{[\s\S]*?min-width:\s*0;/);
