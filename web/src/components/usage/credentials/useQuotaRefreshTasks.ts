...
export function buildQuotaRefreshTaskErrorUpdate(authIndex: string, error: unknown, onAuthRequired?: () => void): { authIndex: string; settled: boolean; stateUpdate: QuotaState } {
  if (error instanceof ApiError && error.status === 401) {
    ...
    return {
      ...
      stateUpdate: {
        refreshStatus: 'failed',
        error: i18n.t('usage_stats.credentials_refresh_error_unauthorized', { defaultValue: '请重新登录后刷新限额。' }),
      },
    }
  }
  return {
...
export function quotaRefreshDisplayError(error?: string): string {
  switch (error) {
    case 'duplicate':
      return i18n.t('usage_stats.credentials_refresh_error_duplicate', { defaultValue: '该凭证的限额刷新正在运行。' })
    case 'duplicate_request':
      return i18n.t('usage_stats.credentials_refresh_error_duplicate_request', { defaultValue: '该凭证已包含在本次刷新请求中。' })
    case 'not_auth_file':
      return i18n.t('usage_stats.credentials_refresh_error_not_auth_file', { defaultValue: '限额刷新仅支持本地认证文件。' })
    case 'unsupported':
      return i18n.t('usage_stats.credentials_refresh_error_unsupported', { defaultValue: '该凭证类型不支持限额刷新。' })
    case 'not_found':
      return i18n.t('usage_stats.credentials_refresh_error_not_found', { defaultValue: '该凭证已不可用。' })
    case 'invalid':
      return i18n.t('usage_stats.credentials_refresh_error_invalid', { defaultValue: '该凭证无法刷新。' })
  }
  return error || i18n.t('usage_stats.credentials_refresh_error_failed', { defaultValue: '限额刷新失败，请稍后重试。' })
}
