import type { AuthFileCooldown, UsageCredentialHealth, UsageIdentity, UsageQuotaCheckResponse, UsageQuotaRow } from '@/lib/types'
...
export type PlanTypeTone = 'free' | 'team' | 'plus' | 'pro' | 'k12' | 'neutral'
...
export function buildAuthFileCredentialRows(
  ...
  cooldowns: Map<string, AuthFileCooldown> = new Map(),
  ...
  return identities.map((identity) => {
    ...
    const planType = firstNonEmpty(...quota.map((row) => row.planType ?? row.plan_type), identity.plan_type)
    ...
    return {
      ...
      workspaceNameLabel: credentialWorkspaceNameLabel(planType, identity.workspace_name),
      ...
      quota,
      quotaResetCreditsAvailableCount: quotaResponse?.rateLimitResetCreditsAvailableCount ?? quotaResponse?.rate_limit_reset_credits_available_count,
      quotaLoading: state?.quotaLoading ?? false,
      quotaError: state?.quotaError,
      refreshStatus: state?.refreshStatus,
      quotaResetting: state?.quotaResetting ?? false,
      cooldown: cooldowns.get(identity.identity),
      displayQuotas,
...
function toDisplayQuota(row: UsageQuotaRow): DisplayQuota | undefined {
  ...
  return {
    ...
    limit,
    remaining,
    resetText: row.resetAt ?? row.reset_at,
    windowSeconds,
...
function knownWindowLabel(label: string, replacement: string): string {
  if (label === '5h' || label === 'Weekly' || label === 'Monthly' || label === 'Window' || label === '每周' || label === '每月' || label === '窗口') {
    return replacement
  }
  for (const candidate of ['5h', 'Weekly', 'Monthly', 'Window', '每周', '每月', '窗口']) {
...
function genericWindowLabel(label: string): boolean {
  return /\bWindow\b/.test(label) || label.includes('窗口')
}
...
function quotaPercent(row: UsageQuotaRow, used?: number, limit?: number): { percent: number | null; kind: DisplayQuota['percentKind'] } {
  ...
  const usedPercent = finiteNumber(row.usedPercent ?? row.used_percent)
  if (usedPercent !== undefined) {
    return { percent: clampPercent(usedPercent), kind: 'used' }
  }
  const remainingFraction = finiteNumber(row.remainingFraction ?? row.remaining_fraction)
...
function quotaStatus(row: UsageQuotaRow, percent: number | null, kind: DisplayQuota['percentKind']): QuotaStatus {
  if (row.limitReached ?? row.limit_reached) {
    return 'danger'
  }
...
function credentialAuthTypeLabel(identity: UsageIdentity): string {
  return firstNonEmpty(identity.auth_type_name) ?? (identity.auth_type === 1 ? '认证文件' : 'AI 供应商')
}
...
function credentialWorkspaceNameLabel(planType?: string, workspaceName?: string): string | undefined {
  if (credentialPlanTypeTone(planType) !== 'team') {
    return undefined
  }
  return firstNonEmpty(workspaceName)
}
