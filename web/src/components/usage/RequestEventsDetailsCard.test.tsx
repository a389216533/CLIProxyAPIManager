...
describe('RequestEventsDetailsCard pagination', () => {
  it('omits the repeated Request Event title block', () => {
    const html = renderCard();

    expect(html).not.toContain('请求事件日志');
    expect(html).not.toContain('共 120 条事件');
    expect(html).not.toContain('筛选、查看并导出页面中各项聚合数据背后的原始请求事件。');
  ...
  it('renders total events, current page, page size options, and disabled page buttons', () => {
    ...
    expect(html.indexOf('>时间</th>')).toBeLessThan(html.indexOf('>来源</th>'));
    expect(html.indexOf('>时间</th>')).toBeLessThan(html.indexOf('>API Key</th>'));
    expect(html.indexOf('>API Key</th>')).toBeLessThan(html.indexOf('>来源</th>'));
    expect(html.indexOf('>来源</th>')).toBeLessThan(html.indexOf('>模型</th>'));
    expect(html.indexOf('>模型</th>')).toBeLessThan(html.indexOf('title="推理强度">推理强度</th>'));
    expect(html.indexOf('title="推理强度">推理强度</th>')).toBeLessThan(html.indexOf('>速度模式</th>'));
    expect(html.indexOf('>速度模式</th>')).toBeLessThan(html.indexOf('>结果</th>'));
    expect(html.indexOf('>结果</th>')).toBeLessThan(html.indexOf('>类型</th>'));
    expect(html.indexOf('>类型</th>')).toBeLessThan(html.indexOf('>端点</th>'));
    expect(html.indexOf('>端点</th>')).toBeLessThan(html.indexOf('title="首字延迟">首字延迟</th>'));
    expect(html.indexOf('title="首字延迟">首字延迟</th>')).toBeLessThan(html.indexOf('title="使用 latency_ms，单位 毫秒">总延迟</th>'));
    expect(html.indexOf('title="使用 latency_ms，单位 毫秒">总延迟</th>')).toBeLessThan(html.indexOf('>输入</th>'));
    expect(html).not.toContain('title="首字后可见输出 token 的平均速度">生成速度</th>');
    ...
    expect(html).toMatch(/<td class="[^"]*requestEventsNoWrapCell[^"]*">快速<\/td>/);
    expect(html).toMatch(/<td class="[^"]*requestEventsNoWrapCell[^"]*">SSE<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*" title="\/messages">\/messages<\/td>/);
    expect(html.indexOf('>45毫秒</td>')).toBeLessThan(html.indexOf('>120毫秒</td>'));
    expect(html).not.toContain('30.0 t/s');
  ...
  it('maps request speed mode values and falls back for missing values', () => {
    ...
    expect(countOccurrences(html, '>快速</td>')).toBe(2);
  ...
  it('keeps the TTFT column visible when TTFT is missing', () => {
    ...
    expect(html.indexOf('title="首字延迟">首字延迟</th>')).toBeLessThan(html.indexOf('title="使用 latency_ms，单位 毫秒">总延迟</th>'));
    expect(html).toMatch(/成功<\/span><\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">SSE<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*" title="\/messages">\/messages<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">-<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">120毫秒<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">100<\/td>/);
  });
  ...
  it('keeps the Latency column visible when latency is missing', () => {
    ...
    expect(html.indexOf('title="首字延迟">首字延迟</th>')).toBeLessThan(html.indexOf('title="使用 latency_ms，单位 毫秒">总延迟</th>'));
    expect(html.indexOf('title="使用 latency_ms，单位 毫秒">总延迟</th>')).toBeLessThan(html.indexOf('>输入</th>'));
    expect(html).toMatch(/45毫秒<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">--<\/td><td class="[^"]*requestEventsNoWrapCell[^"]*">100<\/td>/);
  });
  ...
  it('renders cache rate after cached tokens with two decimal places', () => {
    ...
    expect(html.indexOf('>缓存</th>')).toBeLessThan(html.indexOf('>缓存率</th>'));
    expect(html.indexOf('>缓存率</th>')).toBeLessThan(html.indexOf('>Token 总数</th>'));
  ...
  it('uses backend source values while showing resolved source labels', () => {
    ...
    expect(html).toContain('aria-label="来源"><span class="_triggerText_c80422 ">Team Prefix</span>');
  });
  ...
  it('uses backend model and source options instead of current page grouping', () => {
    ...
    expect(html).toContain('aria-label="模型"><span class="_triggerText_c80422 ">claude-opus</span>');
    expect(html).toContain('aria-label="来源"><span class="_triggerText_c80422 ">Provider B</span>');
  });

  it('renders a Result filter and no Credential filter control', () => {
    const html = renderCard({ resultFilter: 'failed' });

    expect(html).toContain('aria-label="结果"');
  ...
  it('keeps filters, actions, and pager controls in their compact toolbars', () => {
    const html = renderCard();

    expect(html).toContain('_requestEventsFiltersGroup_');
    expect(html).toContain('_requestEventsActions_');
    expect(html.indexOf('_requestEventsFiltersGroup_')).toBeLessThan(html.indexOf('_requestEventsActions_'));
    expect(html).not.toContain('_requestEventsTitleRow_');
    expect(html).not.toContain('_requestEventsCountBadge_');
  ...
  it('renders one export menu trigger instead of separate CSV and JSON buttons', () => {
    ...
    expect(countOccurrences(html, '>导出<')).toBe(1);
    expect(html.indexOf('清除筛选')).toBeLessThan(html.indexOf('>导出<'));
  ...
  it('shows a dash when backend cost is unavailable', () => {
    ...
    expect(html).toContain('总成本');
    expect(html).toContain('title="请先设置价格以计算成本">-</td>');
  });

  it('renders a column selector before the page size control', () => {
    const html = renderCard();

    expect(html).toContain('aria-label="列"');
    expect(html.indexOf('aria-label="列"')).toBeLessThan(html.indexOf('<span>大小</span>'));
  ...
  it('can render only the selected request event columns', () => {
    ...
    expect(html).toContain('>模型</th>');
    expect(html).toContain('>总成本</th>');
    ...
    expect(html).not.toContain('<th>来源</th>');
    expect(html).not.toContain('title="首字延迟">首字延迟</th>');
    expect(html).not.toContain('title="使用 latency_ms，单位 毫秒">总延迟</th>');
  ...
  it('honors controlled request event column selection', () => {
    ...
    expect(html).toContain('<td class="_modelCell_');
    expect(html).not.toContain('<th>API Key</th>');
    expect(html).not.toContain('>总成本</th>');
