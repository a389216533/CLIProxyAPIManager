...
import { IconDiamond, IconDollarSign, IconSatellite, IconChevronDown, IconChevronUp } from '@/components/ui/icons';
...
export function DailyAveragePanel({ usage, loading, reserveVisible = false }: DailyAveragePanelProps) {
  ...
  return (
    <section
      className={`${styles.dailyAveragePanel} ${visible ? styles.dailyAveragePanelVisible : styles.dailyAveragePanelEntering} ${isCollapsed ? styles.dailyAveragePanelCollapsed : ''}`.trim()}
      aria-label={t('usage_stats.daily_average')}
    >
      <div className={styles.dailyAverageIdentity}>
        <div className={styles.dailyAverageTitleRow}>
        ...
        <button
          type="button"
          className={styles.dailyAverageCollapseButton}
          onClick={() => setIsCollapsed(!isCollapsed)}
          aria-expanded={!isCollapsed}
          aria-label={isCollapsed ? t('common.expand') || '展开' : t('common.collapse') || '收起'}
        >
          {isCollapsed ? <IconChevronDown size={14} /> : <IconChevronUp size={14} />}
        </button>
      </div>
      <div className={`${styles.dailyAverageCollapseContent} ${isCollapsed ? styles.dailyAverageContentCollapsed : ''}`.trim()}>
